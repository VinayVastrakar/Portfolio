...
public class OtpServiceImpl implements OtpService {
    ...
    public boolean verifyOtp(String username, String otp) {
        ...
        // Fetch the most recent unverified OTP for the user
        Optional<OtpVerification> otpVerificationOptional = otpVerificationRepository.findTopByUserAndIsVerifiedFalseOrderByExpirationTimeDesc(user);
...
//        remove OTPs for user
    ...
    public void deleteExpiredOtps(){
        ...
        List<OtpVerification> expiredOtps = otpVerificationRepository.findExpiredOtps(now);

        if(!expiredOtps.isEmpty()){
            otpVerificationRepository.deleteAll(expiredOtps);
            logger.info("Deleted {} expired OTP(s).", expiredOtps.size());
        }else{
            logger.info("No expired OTPs found for deletion.");
        }
    }

    public void deleteOtpsByUsers(Users users){
        List<OtpVerification> expiredOtps = otpVerificationRepository.findAllOtpsByUser(users);

        if(!expiredOtps.isEmpty()){
            otpVerificationRepository.deleteAll(expiredOtps);
            logger.info("Deleted {} expired OTP(s) and new Otp Generate.", users.getName());
        }else{
            logger.info("No expired OTPs found for deletion.");
        }
    }
